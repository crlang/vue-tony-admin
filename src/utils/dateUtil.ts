  ...
  switch (v) {
    ...
    default: {
      ...
      if (Number.parseInt(v) > 0) {
        dateRange.start = formatToDate(new Date(nowTime - oneDay * Number.parseInt(v)))
...
export function formatMoney(num: number | string, hasInt = false) {
  ...
  curentNum = hasInt && num < dividend ? Number.parseInt(curentNum) : Number(curentNum).toFixed(2)
