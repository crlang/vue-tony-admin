...
export function handleCustomValidator(rule: InternalRuleItem, component: ComponentType, componentType: HTMLInputElement['type']): EleFormItemRule {
  ...
  function validateValue(rule: any, value: any, component: string, componentType: string, typ: 'min' | 'max' | 'len'): string | null {
    const isNumberType = componentType === 'number'
    const isCheckboxGroup = ['ElCheckboxGroup', 'CustomCheckbox'].includes(component)

    if (isNullOrUnDef(value)) {
      return createRuleMessage(rule, typ)[isNumberType ? 'number' : isCheckboxGroup ? 'array' : 'text']
    }

    if (isNumberType) {
      return value < (rule.min ?? rule.len) || value > (rule.max ?? rule.len)
        ? createRuleMessage(rule, typ).number
        : null
    }
    else if (isCheckboxGroup) {
      return value.length < (rule.min ?? rule.len) || value.length > (rule.max ?? rule.len)
        ? createRuleMessage(rule, typ).array
        : null
    }
    else {
      return value.length !== (rule.len ?? value.length < rule.min ?? value.length > rule.max)
        ? createRuleMessage(rule, typ).text
        : null
    }
  }
  if (Reflect.has(rule, 'min') && Reflect.has(rule, 'max')) {
    ...
    rule.validator = (rule, value, callback) => {
      const msg = validateValue(rule, value, component, componentType, 'minmax')
      if (msg) {
        return callback(new Error(msg))
      }
      callback()
  ...
  else if (Reflect.has(rule, 'min')) {
    rule.required = true
    rule.validator = (rule, value, callback) => {
      const msg = validateValue(rule, value, component, componentType, 'min')
      if (msg) {
        return callback(new Error(msg))
      }
      callback()
  ...
  else if (Reflect.has(rule, 'max')) {
    rule.required = true
    rule.validator = (rule, value, callback) => {
      const msg = validateValue(rule, value, component, componentType, 'max')
      if (msg) {
        return callback(new Error(msg))
      }
      callback()
  ...
  else if (Reflect.has(rule, 'len')) {
    rule.validator = (rule, value, callback) => {
      const msg = validateValue(rule, value, component, componentType, 'len')
      if (msg) {
        return callback(new Error(msg))
      }
      callback()
