<template>
  <PageWrapper title="Tree 函数示例" content-background>
    <div class="mb-4">
      <ElButton @click="checkAll(true)">全选</ElButton>
      <ElButton @click="checkAll(false)">全不选</ElButton>

      <ElButton @click="expandAll(true)">展开全部</ElButton>
      <ElButton @click="expandAll(false)">折叠全部</ElButton>

      <ElButton @click="handleLevel(2, true)">显示第2级</ElButton>
      <ElButton @click="handleLevel(2, false)">隐藏第2级</ElButton>

      <ElButton @click="handleSetExpandData(true)">展开1-1-1节点数据</ElButton>
      <ElButton @click="handleSetExpandData(false)">折叠1-1-1节点数据</ElButton>
    </div>
    <div class="mb-4">
      <ElButton @click="handleSetCheckData">设置勾选数据</ElButton>
      <ElButton @click="handleGetCheckData">获取勾选数据</ElButton>

      <ElButton @click="handleSetNodeData">设置节点数据</ElButton>
      <ElButton @click="handleGetNodeData">获取节点数据</ElButton>
    </div>
    <div class="mb-4">
      <ElButton @click="appendNodeByKey">在0-0-0内添加子节点</ElButton>
      <ElButton @click="insertNodeByKey('before')">在0-0-0前添加节点</ElButton>
      <ElButton @click="insertNodeByKey('after')">在0-0-0后添加节点</ElButton>
      <ElButton @click="updateNodeByKey">更新0-0-0子节点数据</ElButton>
      <ElButton @click="deleteNodeByKey">删除2-2-0节点</ElButton>
    </div>
    <div class="mb-4">
      <ElInput v-model="filterTreeNodeKey" class="mb-2" placeholder="请输入筛选内容" />
    </div>
    <ElCard header="函数操作">
      <ElTree
        ref="treeRef"
